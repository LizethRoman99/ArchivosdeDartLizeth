...
class ListarEspanolScreen extends StatefulWidget {
  const ListarEspanolScreen({super.key});

  @override
  State<ListarEspanolScreen> createState() => _ListarEspanolScreenState();
}
...
class _ListarEspanolScreenState extends State<ListarEspanolScreen> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Lista De Exportaciones'),
      ),
                        ...
                        onPressed: () {
                          showDialog(
                            ...
                            builder: (context) => AlertDialog(
                              title: const Text('Confirman eliminacion'),
                              content:
                                  const Text('¿Estas seguro que deseas eliminar este producto?'),
                              actions: [
                                TextButton(
                                  ...
                                  onPressed: () {
                                    Exportaciones.eliminarExportacion(exportacion.producto)
                                      ...
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(content: Text('Exportacion eliminada')),
                                      );
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Kilogramos'),
              keyboardType: TextInputType.number,
            ),
            TextField(
              controller: precioKiloController,
              decoration: const InputDecoration(labelText: 'Precio del Kilogramo'),
              keyboardType: TextInputType.number,
            ),
            TextField(
              controller: precioDolarController,
              decoration:
                  const InputDecoration(labelText: 'Precio Del Dolar'),
              keyboardType: TextInputType.number,
            ),
          ],
        ),
          ...
          TextButton(
            ...
            final nuevaExportacion = Exportaciones(exportacion.producto,
              ...
              Exportaciones.editarExportacion(nuevaExportacion)
                  .then((_) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Exportacion Editada')),
                );
                Navigator.pop(context);
                setState(() {});
              }).catchError((error) {
